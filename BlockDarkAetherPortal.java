...
public class BlockDarkAetherPortal extends BlockPortalBase
{
	//Look in BlockPortalBase.java to see the full array of hooks you can use for your portal block.

	public BlockDarkAetherPortal(int i)
	{
		super(i, Block.portal.blockIndexInTexture, Material.portal);
	...
	{
		return new WorldProviderDarkAether();
	}

	public Teleporter getTeleporter()
	{
		return new DarkAetherTeleporter();
	}
	...
	public List canTeleportFromDimension()
	{
		...
		arraylist.add(Integer.valueOf(35));//player can teleport from Nether to this dimension
		return arraylist;
	}
	/**
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
	{
		...
		if (par1World.getBlockId(par2 - 1, par3, par4) == mod_MetroidCraft.lightBlock.blockID || par1World.getBlockId(par2 + 1, par3, par4) == mod_MetroidCraft.lightBlock.blockID)
		{
			i = 1;
		}

		if (par1World.getBlockId(par2, par3, par4 - 1) == mod_MetroidCraft.lightBlock.blockID || par1World.getBlockId(par2, par3, par4 + 1) == mod_MetroidCraft.lightBlock.blockID)
		{
			j = 1;
		}
		...
		for (int k = -1; k <= 2; k++)
		{
			for (int i1 = -1; i1 <= 3; i1++)
			{
				...
				if (flag)
				{
					if (k1 != mod_MetroidCraft.lightBlock.blockID)
					{
						return false;
					}
				...
				//the igniter
				if (k1 != 0 && k1 != mod_MetroidCraft.lightBlock.blockID)
				{
					return false;
				}
			}
		...
		for (int l = 0; l < 2; l++)
		{
			...
			{
				par1World.setBlockWithNotify(par2 + i * l, par3 + j1, par4 + j * l, mod_MetroidCraft.darkAetherPortal.blockID);
			}
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
	{
		...
		if (par1World.getBlockId(par2, k - 1, par4) != mod_MetroidCraft.lightBlock.blockID)
		{
		...
		if (l != 3 || par1World.getBlockId(par2, k + l, par4) != mod_MetroidCraft.lightBlock.blockID)
		{
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		}
		...
		if ((par1World.getBlockId(par2 + i, par3, par4 + j) != mod_MetroidCraft.lightBlock.blockID || par1World.getBlockId(par2 - i, par3, par4 - j) != blockID) && (par1World.getBlockId(par2 - i, par3, par4 - j) != mod_MetroidCraft.lightBlock.blockID || par1World.getBlockId(par2 + i, par3, par4 + j) != blockID))
		{
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		}
	...
	{
		return "Entering The Dark Aether";
	}


	public String getLeavingMessage() 
	{
		return "Exiting The Dark Aether";
	}
	
	public void afterTeleportToDimension(Minecraft minecraft, EntityPlayerSP entityplayersp)
    {
		minecraft.theWorld.playSoundAtEntity(entityplayersp, "metroid.dangerPoison", 1.0F, 1.0F);
    }
